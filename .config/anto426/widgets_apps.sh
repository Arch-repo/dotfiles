...
write_custom_widget_hypr_rules() {
...

    {
        ...
        for id in $(custom_widget_ids); do
            ...
            # Skip custom widgets in app mode (their rules are passed directly during launch)
            [[ "$mode" == "terminal" ]] || continue
...
launch_custom_widget() {
    ...
    if [[ "$mode" == "terminal" ]]; then
        ...
        local w h x y pid_f
        pid_f="${XDG_RUNTIME_DIR:-/tmp}/anto426-widgets/$id.pid"
        w="$(custom_widget_meta "$id" w 2>/dev/null || printf '520')"
        h="$(custom_widget_meta "$id" h 2>/dev/null || printf '360')"
        x=""
        y=""
        ...
        [[ -n "$x" && -n "$y" ]] || {
            x="$(custom_widget_meta "$id" dx 2>/dev/null || printf '40')"
            y="$(custom_widget_meta "$id" dy 2>/dev/null || printf '72')"
        }
        
        # Build rules string
        rules="float;size $w $h;move $x $y;rounding 14;opacity 0.94 0.88;border_size 1;no_anim"
        [[ -n "$monitor" ]] && rules="monitor $monitor silent;$rules"
        
        # If widgets are locked, apply lock rules initially
        if declare -F widgets_locked >/dev/null 2>&1 && widgets_locked; then
            rules="$rules;no_focus;no_follow_mouse;no_initial_focus;focus_on_activate off"
        fi
        
        # Launch with our PID wrapper
        hyprctl dispatch exec "[$rules] bash -c 'echo \$\$ > \"$pid_f\"; exec $command'" >/dev/null 2>&1 &
    fi
}
