...
write_terminal_theme() {
    # Write the theme palette file (used as fallback reference)
...
EOF
    ...
    # Write colors DIRECTLY into the main Ghostty config so it hot-reloads reliably.
    # We maintain a generated block between markers; everything else in the config is preserved.
...
EOF
    ...
    # Remove any previously generated color block
    sed -i '/# <<< anto426-dynamic-colors/,/# anto426-dynamic-colors >>>/d' "$ghostty_config"

    # Append the dynamic color block at the end of the config
    cat >>"$ghostty_config" <<EOF

# <<< anto426-dynamic-colors (generated by wallpaper_effects.sh - do not edit)
background = ${background#\#}
foreground = ${foreground#\#}
cursor-color = ${accent#\#}
cursor-text = ${background#\#}
