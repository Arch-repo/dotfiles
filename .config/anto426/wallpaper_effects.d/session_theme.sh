...
enable_zen_profile_theme() {
    ...
    if ! grep -Fq 'toolkit.legacyUserProfileCustomizations.stylesheets' "$user_js" 2>/dev/null; then
...
write_zen_theme() {
    ...
    local chrome_import="@import url(\"file://$zen_chrome_css\");"
    local content_import="@import url(\"file://$zen_content_css\");"
    local root profile
...
EOF
    ...
    for root in \
        ...
        "$HOME/.var/app/io.github.zen_browser.zen/.zen"; do
        [[ -d "$root" ]] || continue
